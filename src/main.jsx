...
import { 
  Twitter, Instagram, RefreshCw, Send, Settings, X, Save, Camera, Sparkles 
} from 'lucide-react';

// REMPLACEZ CECI PAR VOTRE CONFIGURATION FIREBASE CONSOLE
...
function App() {
  ...
  useEffect(() => {
    signInAnonymously(auth).catch(() => setSyncStatus('local'));
    return onAuthStateChanged(auth, setUser);
  }, []);

  useEffect(() => {
    if (!user) return;
    // Note: 'moodboard-data' est le nom de la collection pour votre version GitHub
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[1000] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-3xl p-8 shadow-2xl">
            ...
            <form onSubmit={(e) => {
              ...
              <input name="status" placeholder="Statut" defaultValue={content.status} className="w-full border-2 border-zinc-100 p-3 rounded-xl outline-none" />
              <input name="profilePic" placeholder="URL Image" defaultValue={content.profilePic} className="w-full border-2 border-zinc-100 p-3 rounded-xl outline-none" />
              <input name="twitter" placeholder="Texte Twitter" defaultValue={content.twitterPreview} className="w-full border-2 border-zinc-100 p-3 rounded-xl outline-none" />
              <button type="submit" className="w-full bg-zinc-900 text-white py-4 rounded-xl font-black uppercase tracking-widest flex items-center justify-center gap-2">
